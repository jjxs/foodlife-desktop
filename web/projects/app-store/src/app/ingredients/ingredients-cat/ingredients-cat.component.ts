...
@Component({
  selector: 'app-ingredients-cat',
  templateUrl: './ingredients-cat.component.html',
  styleUrls: ['./ingredients-cat.component.css']
})
export class IngredientsCatComponent implements OnInit, AfterViewInit {
  ...
  onSearch() {
    ...
    this.json.post('s/ingredients_api/get_cat_data', {}).subscribe((response: JsonResult) => {
      if (response) {
        this.empty.refreshFlg = true;
        this.dataSource.data = response.data;
        // setTimeout(function () {
        //   var button_num = document.getElementsByClassName("mat-icon-button mat-button-base ng-star-inserted")
        //   for (var i = 1; i < button_num.length; i++) {
  ...
  doDelete(row) {
    this.cmp.confirm('選択されたカテゴリーを削除しますか？').afterClosed().subscribe(result => {
      if (result === 'yes') {
          ...
          this.json.post('s/ingredients_api/delete_cat_data', params).subscribe((response: JsonResult) => {
            if (response.result) {
              this.cmp.pop(response.message);
              this.onSearch();
            }
