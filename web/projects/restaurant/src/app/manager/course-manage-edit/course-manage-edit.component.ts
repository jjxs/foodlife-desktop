...
@Component({
  selector: 'course-manage-edit',
  templateUrl: './course-manage-edit.component.html',
  styleUrls: ['./course-manage-edit.component.css']
})
export class CourseManageEditComponent implements OnInit {
  menus = null;
  categorys = []
  master_group = [];
  orign_menus = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<CourseManageEditComponent>,
  ...
  getCourse(id) {
    ...
    this.jsonSrv.get('restaurant/menu/get_course/', {course_id: id}).subscribe((response) => {
      ...
      this.menu_course_detail = response['menu_course_detail']
    })
  }
  ...
  addMenu(event, menu) {
    this.menu_course_detail.push(menu);
    this.changeMenuData(this.orign_menus);
  }
  ...
  save(event) {
    let course_detail_menu_id = [];
    this.menu_course_detail.forEach((item: any) => {
      course_detail_menu_id.push(item['id']);
    });
    let params = {
      'course_menu_id': this.course_menu_id,
      'course_detail_menu_id' : course_detail_menu_id,
      'id': this.id
    }
    this.jsonSrv.post('restaurant/menu/post_course/', params).subscribe((response?) => {
